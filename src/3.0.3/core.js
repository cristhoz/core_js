...
(function(factory) {
	...
	Core.parents = function(el, validate) {
		...
		var _find = function() {
			if(!Core.is(el, validate) && !Core.is(el, 'body')) {
				el = el.parentNode;
	...
	Core.cssClass = (function(cssClass) {
		...
		cssClass.remove = function(el, className) {
			...
			for(var i = 0, len = arr.length; i < len; i++) {
				if(arr[i].trim() !== '' && all.indexOf(arr[i].trim()) >= 0) {
	...
	/**
	 * This method is forEach for Arrays or NodeList
	 * @param  {Array}    ar parameter for the Array or NodeList
	...
	Core.forEach = function(ar, fn) {
		...
		if(!Core.isFunction(fn)) {
			throw 'Function callback is required';
		}

		Array.prototype.forEach.call(ar, fn);
	};
	...
	Core.ajax = function(options, failed) {
		...
		var _xhr = function() {
			...
			if(Core.isObject(options.headers)) {
	...
	Core.routes = function(routes, options) {
		...
		var _getRoutes = function() {
			...
			for(var prop in routes) {
				...
				if((!!pathname || !!hash) && (isPathname || isHash)) {
					...
					routesArr = (isHash) ? prop.substring(prop.indexOf('/')) : prop;
					routesArr = routesArr.split('/');
					...
					for(var i = 0, len = routesArr.length; i < len; i++) {
						if((isPathname && pathname[i] === routesArr[i]) || (isHash && hash[i] === routesArr[i])) {
							thisIs = true;
							continue;
						}
					}
					...
					if(prop === '/' && (path_url === dataURL.href || path_url + '#!/' === dataURL.href)) {
						_call(routes[prop], {
							hash: _getHash(),
							search: _getSearch()
						}, []);
		...
		var _setOptions = function() {
			if(Core.isObject(options)) {
				if(Core.isBoolean(options.watch) && options.watch) {
					window.addEventListener('hashchange', function() {
						if(Core.isFunction(options.isChange)) {
							if(options.isChange()) {
								_getRoutes();
							}
						} else {
	...
	Core.validateForm = function(form, options) {
		...
		var _submit = function() {
			...
			if(Core.isString(options.trigger) || Core.isElementHTML(options.trigger)) {
				var $trigger = Core.isString(options.trigger) ? document.querySelector(options.trigger) : options.trigger;
	...
	Core.calculateRatio = function(srcWidth, srcHeight, maxWidth, maxHeight, toMax) {
		var ratio = [ maxWidth / srcWidth, maxHeight / srcHeight ];
		ratio = (toMax) ? Math.max(ratio[0], ratio[1]) : Math.min(ratio[0], ratio[1]);
