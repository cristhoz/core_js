...
(function(factory) {
    ...
    Core.UNIX_HOUR = 36e5;
    Core.UNIX_DAY = 864e5;
    Core.UNIX_WEEK = 6048e5;
    Core.UNIX_MONTH = 2629743e3;
    Core.UNIX_YEAR = 31556926e3;
    ...
    Core.config = function(config) {
        ...
        config.PATH_URL = protocol + "//" + hostname + (port ? ":" + port : "");
        //Path URL develop
        config.ROUTE_TEMPLATES = "assets/templates/";
        //Route files of templates
        config.TYPE_DATA_AJAX = "json";
        //Type data require in AJAX
        config.TYPE_REQUEST_AJAX = "POST";
        //Type request in AJAX
        //config.FACEBOOK_ID_APP = '840570035983161'; //ID app in facebook
        config.MIN_AGE = 5;
        //Min age accept in site
        //Config share
    ...
    /**
	 * Methods common and useful in Core
	 */
    ...
    Core.uniqID = function() {
        var num = Math.floor(Date.now() / 1e3) - Math.floor(Math.random() * 65536 + 1);
    ...
    Core.offset = function(el) {
        ...
        while (el && !isNaN(el.offsetLeft) && !isNaN(el.offsetTop) && el != body) {
            var computed = window.getComputedStyle(el);
            var transform = computed.getPropertyValue("-webkit-transform") || computed.getPropertyValue("-ms-transform") || computed.getPropertyValue("transform") || "none";
            var minus = [ 0, 0 ];
            var add = [ 0, 0 ];
    ...
    Core.ajax = function(options) {
        if (!Core.isString(options.url)) {
            return console.error({
                message: "It requires the URL request"
            });
        }
        var _options;
